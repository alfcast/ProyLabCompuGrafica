...
void CFiguras::prismaM(float altura, float largo, float profundidad)  //Funcion creacion prisma
{
...
void CFiguras::prismaT(float altura, float largo, float profundidad, GLint general, GLint front, GLint back, GLint right, GLint left, GLint top, GLint bottom)
{
	GLfloat vertice[8][3] = {
		{ 0.5*largo ,-0.5*altura, 0.5*profundidad },    //Coordenadas Vértice 1 V1
	{ -0.5*largo ,-0.5*altura , 0.5*profundidad },    //Coordenadas Vértice 2 V2
	...
	if (front != NULL)
	{
		...
		glBindTexture(GL_TEXTURE_2D, general);
	}
